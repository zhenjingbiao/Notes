...
public class DatabaseOperation {
    ...
    public void insert_db(String title,String text,String mood,String time){
    	...
    	if(text.isEmpty()){
    		...
    		db.execSQL("insert into notes(title,context,mood,time) values('"+ title+"','"+ text+ "','"+ mood+ "','"+time+"');");
        	//Toast.makeText(context, "插入成功", Toast.LENGTH_LONG).show();
    	}
    	
    }
    public void update_db(String title,String text,String mood,String time,int item_ID){
    	if( text.isEmpty()||mood.isEmpty()){
    		...
    		db.execSQL("update notes set context='"+text+ "',title='"+title+"',mood='"+mood+"',time='"+time+"'where _id='" + item_ID+"'");
